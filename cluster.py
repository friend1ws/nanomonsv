...
def cluster_junction(input_file, output_file, check_margin_size = 100):
    ...
    with gzip.open(input_file, 'rt') as hin:
        ...
        for line in hin:
            ...
            # if F[6] == "de517224-65cc-483a-84bd-4e81e5893964":
...
def filt_clustered_junction1(input_file, output_file, read_num_thres = 3, median_mapQ_thres = 40, max_overhang_size_thres = 300):
...
def filt_clustered_junction2(input_file, output_file, control_junction_bedpe, max_read_num_thres = 0):
    ...
    with open(input_file, 'r') as hin:
        for line in hin:
            ...
            if control_junction_bedpe is not None:
                ...
                try:
                    records = control_junction_db.fetch(F[0], int(F[1]) - 5, int(F[2]) + 5)
                except:
                ...
                if not tabix_error_flag:
                    for record_line in records:
                        ...
                        if F[0] == record[0] and int(F[2]) >= int(record[1]) and int(F[1]) <= int(record[2]) and \
                          F[3] == record[3] and int(F[5]) >= int(record[4]) and int(F[4]) <= int(record[5]):
                            control_flag = True
