...
def gather_local_read_for_realignment(sv_file, alignment_file, output_file, reference_fasta,
    ...
    rname2key = {}
    key2rname2info = {}  # {key: {rname: [mapq1, mapq2, hp_bp1, hp_bp2]}}  hp: 0=unphased, 1=hap1, 2=hap2
    with open(sv_file, 'r') as hin:
        for line in hin:
            ...
            for read in bam_subsample_fetch(alignment_h, tchr1, max(tpos1 - 100, 0), tpos1 + 100):
                ...
                if read.qname not in key2rname2info[key]: key2rname2info[key][read.qname] = [None, None, 0, 0]
            ...
            for read in bam_subsample_fetch(alignment_h, tchr2, max(tpos2 - 100, 0), tpos2 + 100):
                ...
                try:
                    key2rname2info[key][read.qname][3] = read.get_tag("HP")
                except KeyError:
                    pass
    ...
    for read in alignment_h.fetch():
        ...
        if read.qname in rname2key:
            ...
            for key in rname2key[read.qname]:
                mapq1, mapq2, hp_bp1, hp_bp2 = key2rname2info[key][read.qname]
                print(f"{key}\t{read.qname}\t{mapq1}\t{mapq2}\t{hp_bp1}\t{hp_bp2}\t{read_seq}", file = hout)
...
class Alignment_counter(object):
    ...
    def add_long_read_seq(self, treadid, tseq, tmapq1, tmapq2, thp_bp1, thp_bp2):

        self.readid2mapq[treadid] = [int(tmapq1) if tmapq1 != "None" else None, int(tmapq2) if tmapq2 != "None" else None]
        self.readid2hp_bp1[treadid] = int(thp_bp1)
        self.readid2hp_bp2[treadid] = int(thp_bp2)
    ...
    def count_alignment_and_print(self):
        ...
        hp1_count_bp1 = sum(1 for r in supporting_reads if self.readid2hp_bp1.get(r, 0) == 1)
        hp2_count_bp1 = sum(1 for r in supporting_reads if self.readid2hp_bp1.get(r, 0) == 2)
        hp0_count_bp1 = len(supporting_reads) - hp1_count_bp1 - hp2_count_bp1

        hp1_count_bp2 = sum(1 for r in supporting_reads if self.readid2hp_bp2.get(r, 0) == 1)
        hp2_count_bp2 = sum(1 for r in supporting_reads if self.readid2hp_bp2.get(r, 0) == 2)
        hp0_count_bp2 = len(supporting_reads) - hp1_count_bp2 - hp2_count_bp2

        tchr1, tpos1, tdir1, tchr2, tpos2, tdir2, tinseq, tid = self.temp_key.split(',')
        print(f"{tchr1}\t{tpos1}\t{tdir1}\t{tchr2}\t{tpos2}\t{tdir2}\t{tinseq}\t{tid}\t{len(all_rnames)}\t{len(supporting_reads)}\t{hp1_count_bp1}\t{hp2_count_bp1}\t{hp0_count_bp1}\t{hp1_count_bp2}\t{hp2_count_bp2}\t{hp0_count_bp2}", file = self.hout_count)
    ...
    def count_alignment_and_print_sbnd(self):
        ...
        hp1_count = sum(1 for r in supporting_reads if self.readid2hp_bp1.get(r, 0) == 1)
        hp2_count = sum(1 for r in supporting_reads if self.readid2hp_bp1.get(r, 0) == 2)
...
def count_sread_by_alignment(sv_file, alignment_file, output_count_file, output_alignment_info_file, reference_fasta, 
    ...
    with open(output_count_file + ".tmp.local_read_for_realignment", 'r') as hin:
        for line in hin:
            tkey, treadid, tmapq1, tmapq2, thp_bp1, thp_bp2, tseq = line.rstrip('\n').split('\t')
            ...
            alignment_counter.add_long_read_seq(treadid, tseq, tmapq1, tmapq2, thp_bp1, thp_bp2)
    ...
    with open(output_count_file_sbnd + ".tmp.local_read_for_realignment", 'r') as hin:
        for line in hin:
            ...
            alignment_counter_sbnd.add_long_read_seq(treadid, tseq, tmapq1, tmapq2, thp, "0")
